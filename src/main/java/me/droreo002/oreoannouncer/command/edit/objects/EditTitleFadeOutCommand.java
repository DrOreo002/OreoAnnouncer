...
public class EditTitleFadeOutCommand extends EditCommand {

    public EditTitleFadeOutCommand() {
        super("edittitlefadeout", "title_fade_out");
    ...
    protected void execute(OreoAnnouncer main, Player player, String[] args, Announcement an) {
        if (args.length == 1) {
            error(player);
            player.sendMessage(main.getPrefix() + "Usage : /oan editor " + an.getName() + " title_fade_out " + " <integer>");
            return;
        }
        int number = 0;
        try {
        ...
        success(player);
        an.setTitleFadeOut(number);
        player.sendMessage(main.getPrefix() + "Title fade out has been set to " + number + " (In ticks)");
